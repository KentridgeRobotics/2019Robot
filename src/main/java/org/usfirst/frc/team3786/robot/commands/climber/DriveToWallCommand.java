...
public class DriveToWallCommand extends Command {
	...
	protected void execute() {
		if (goForward) { // do we want to go forward?
			if (UltrasonicSensor.getInstance(Side.LEFT).getDistanceCM() > targetDist | UltrasonicSensor.getInstance(Side.RIGHT).getDistanceCM() > targetDist) { // is the sensor farther than desired?
				NeoDriveSubsystem.getInstance().gyroStraight(speed, targetHeading);
				System.err.println("Right ultrasonic distance to wall:" + UltrasonicSensor.getInstance(Side.RIGHT).getDistanceCM());
				System.err.println("Left ultrasonic distance to wall:" + UltrasonicSensor.getInstance(Side.LEFT).getDistanceCM());
			} else {
				isDone = true;
			}
		} else { // we're going backwards
			if (UltrasonicSensor.getInstance(Side.LEFT).getDistanceCM() < targetDist | UltrasonicSensor.getInstance(Side.RIGHT).getDistanceCM() < targetDist) { // is the sensor closer than desired?
				if (speed > 0) { // this is just to see if someone puts a negative number for speed when we're
									// going backwards. A failsafe of sorts
					speed = -1 * speed;
				}
				NeoDriveSubsystem.getInstance().gyroStraight(speed, targetHeading);
				System.err.println("Right distance to wall:" + UltrasonicSensor.getInstance(Side.RIGHT).getDistanceCM());
				System.err.println("Left distance to wall:" + UltrasonicSensor.getInstance(Side.LEFT).getDistanceCM());
			} else {
				isDone = true;
			}
		}
